...
export class InicioPage implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private productSvc: ProductService, private hostRef: ElementRef, private cart: CartService, private nav: NavController) {}
  ...
  openProduct(id?: string | number) {
    if (!id) return;
    // navegar a detalle
    this.nav.navigateForward(['/producto', String(id)]);
  }
}